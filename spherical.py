# copied from https://github.com/mwoedlinger/n-sphere-pytorch/blob/main/transformations.py
import torch
...
def to_spherical(coords: torch.Tensor) -> torch.Tensor:
    ...
    phi_final = torch.arccos(coords[..., -2:-1] / phi_norms[..., -2:-1]) + \
                (2 * torch.pi - 2 * torch.arccos(coords[..., -2:-1] / phi_norms[..., -2:-1])) * (coords[..., -1:] < 0)
...
def to_cartesian(coords: torch.Tensor) -> torch.Tensor:
    ...
    phi_sin_prod = torch.prod(phi_lower + torch.triu(torch.ones((*phi.shape, n - 1), device=coords.device), diagonal=1),
                              dim=-1)
